...
class Upload extends React.Component {
    ...
    maxSelectFile(event) {
        ...
        if (files.length > 1) {
            ...
            if (err !== '') {
                // error caught
                event.target.value = null;
                toast.error(err + " is/are too large. Please select file < 50Mb")
            }
        }
        return true;
    }
    ...
    fileUploadHandler(event) {
        ...
        axios.post('http://localhost:3333/api/upload', data, {
            ...
            toast.error(`Upload failed with status: ${err.statusText}`);
        });
    }
                ...
                <div className="container mt-5">
                    ...
                    <form method="post" name="videoUpload" action="/api/upload" id="#" encType="multipart/form-data">
                        <div className="form-group files">
                            ...
                            onClick={this.fileUploadHandler.bind(this)}>Upload Videos
                            </button>
                        </div>
                    </form>
                </div>
